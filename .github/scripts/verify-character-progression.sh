...
first_signup="$(signup_test_user "$email_one" "$password")"
second_signup="$(signup_test_user "$email_two" "$password")"
...
test "$telemetry" = 'system|ci.progression|progression.integration|3|1|5|5|true'
...
cap_threshold="$(docker exec "$db_container" psql -U postgres -d postgres -Atqc "
  select cumulative_xp_by_level[max_level] from app_private.level_progression_curves where version = 3;")"
