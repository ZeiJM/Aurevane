...
### Phase 5 presentation delivery slice — 2026-09-25

Branch: `agent/phase5-map-identity-presentation-20260925`. Presentation implementation `f06f1426`, reconciled with current main `cd86b4f0` at `86d2491d`; final review/test follow-up and this evidence are subsequent commits on the same branch. This is a review candidate, not a deployment or full P5-W2/W3/W4 acceptance.

- **Freshness and ownership:** refreshed main and inspected active Phase 5 PRs. Preserved the merged #731 roadmap/two-chat contract and #724 public-avatar normalization. Incorporated only the exact #730 Profile-choice prerequisite (`cd0646ea49e5e773956e6d9e7bb0d49d8635b23d`), which remains open. The candidate contains that prerequisite until it lands; do not treat its existing server-loading work as a new competing implementation. Did not incorporate #681 geography, #712/#715 crossings/wilderness, #718/#719 Anchor, #720/#729 Drift, #728 music or #726 Nexus work.
- **P5-W2:** current player area and inspected destination now have separate text. Tile/landmark selection is inspection only; travel and route replacement require explicit buttons using the existing command. Manual inspection survives authoritative sector changes; My Position explicitly resumes following. Existing route/ETA/stop context survives view changes. Region/index selection exposes pressed state and only offers server-projected sectors. No speculative ETA or route-validity claim is added: the existing API provides the committed route after travel starts.
- **P5-W3 presentation:** compact location details reuse approved region summaries and revealed landmark names. Local service and nearby-player actions explicitly name the player's actual area while another destination is inspected; existing server eligibility/objective actions are retained. Existing paintings, map area and visible ambient layers remain. Motion-off now also disables player-marker interpolation; existing reduced-motion behavior remains. No speed, position, collision, interaction-distance or encounter rule changed.
- **P5-W4 presentation:** Primary/Secondary chips name their roles; an expandable build summary explains base-stat versus Skill-library contribution, active Essence/Resonance from the existing build projection, and actual selected/capacity counts. Current Path distinguishes unavailable data, Unawakened without a choice, eligible choice, Ascended and Severed. Permanent exclusivity, persistence through Rekindling and the separate confirmation remain explicit. Existing expected-version/idempotent PUT is retained; stale errors are announced. No unrevealed catalog or invented power mechanics are rendered.
- **Verification:** final `pnpm check` passes formatting, lint, workspace types, **2,899 Vitest tests plus seven Node report/audio checks**, and production builds. Existing world authority/projection/privacy/retry/encounter and supernatural eligibility/exclusivity/replay suites are included. Independent review found two stale browser-test assumptions; all three old Current Path selectors and the Crown Road implicit-travel assumption were corrected without weakening geometry/tick assertions. Added real-projection browser coverage for inspection surviving a player-sector refresh and My Position restoring following.
- **Rendered evidence:** actual Atlas and Profile components with production CSS were exercised in local Chromium at **1440×900, 1366×768, 390×844 and 320×740**. Keyboard selection sent no travel request; explicit travel/stop, retained route/inspection context, globe zoom/label containment, ambient animation, motion-off/reduced-motion, build disclosure, path states, confirmation/cancel and a stale response all passed. No page errors or horizontal document overflow were observed. Desktop Current Path content is reachable through its own scrolling panel. Screenshots were visually inspected. These checks used deterministic public snapshots and simulated transport, not authenticated database acceptance or human approval.
- **Remaining verification:** the updated Living Atlas and Profile-layout Playwright suites still need the disposable Postgres/authenticated CI run. Docker is unavailable in this workspace. Local unit/service security checks and simulated rendering do not substitute for that gate. The Owner explicitly authorized branch publication and a draft PR on 2026-09-25 after the initial automatic-review block. Authenticated CI results must be recorded separately from local evidence.
- **W3 media dependency:** inspected #681 at `702eb44c2f93d8f17935363d86255f9142fce702`, including all eight actual collision rows, safe footprints, road endpoints and landmark positions. It still references v01 paintings; approved matching v02 assets are absent. Its matching-art merge hold stays intact. The candidate-specific checklist is appended to `content/art-requests/ART-WORLD-001-living-atlas.md`; neither geometry nor public art/provenance was altered.
- **W4 contract dependency:** #730 supplies only `state` plus eligible `{transitionId, transitionContentVersion, path}` options. Authored power title/summary, requirements, effects, costs, duration/recovery and counterplay are not in that public projection. The existing Phase 5 chat must supply approved version-pinned, eligibility-filtered facts through that same projection before the presentation chat can render representative mechanics. Do not infer them from IDs or add a parallel endpoint. Broader NPC/vendor/objective content likewise remains the existing content owner's work.

P5-W1 authoritative timing stays with the existing Phase 5 chat. Seamless multi-sector rendering stays P6-W1. The Vercel deployment lock remains `**: false`; no migration or deployment was attempted.
