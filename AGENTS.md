...
- All authoritative game state must be server-side. The browser may request actions, but it must never determine authoritative combat legality, path/movement cost, target validity, action costs, Reaction triggers, cooldowns, terrain transformations, timers, NPC/AI actions, practice-opponent unlocks/config legality, content-discovery/access state, rewards, XP, currency, inventory, progression, offline-training elapsed time/rewards, PvP, trading, quest outcomes, Rekindling state, Veteran Edge state, lore discovery, permissions, Owner overrides, premium prices, payment completion, purchase fulfillment, or other persistent state.
...
- Keep one authoritative source of truth for game rules and live configuration. Do not duplicate formulas, permissions, balance values, unlock requirements, validation, product prices, grant bundles, offline-training accrual rules, combat-AI profile values, Tactical Record requirements, combat Target Specs, action costs, use requirements, effect definitions, terrain rules, acquisition rules, content visibility rules, or authoritative calculations across layers.
...
- Combat Arts, Basic Attack profiles, combat items, scenario actions, and AI legality should reuse the same typed targeting/requirement/effect grammar where practical rather than creating parallel battle rule engines.
...
- Combat content must not permit arbitrary JavaScript/SQL entered in the Master Panel. Requirements and designer formulas should use typed/whitelisted validated grammars with bounded deterministic server execution.
...
- The battle UI is board-first. Keep targeting, movement, objective state, concise actor state, turn order, forecast, and action controls immediately readable; move deep logs/status/terrain detail into contextual inspection rather than permanent oversized sidebars.
...
- Weapons, combat items, summons, battle objects, scenario interactions, and AI actions should reuse the same targeting/effect/rule grammar rather than creating independent combat subsystems.
- PvE and PvP should share core action definitions. Explicit PvP overrides must be inspectable, versioned, tested, and player-documented where relevant rather than hidden copies of the ability.
...
- Battle commands submit intent, never authoritative outcomes. Pin relevant combat/content/AI/scenario/rules versions and RNG state so battles are reproducible enough for replay, support, simulation, and regression testing.
...
- `https://www.theninja-rpg.com/` may be used only as an abstract presentation/UX/game-system quality benchmark. Never copy its implementation, assets, wording, characters, setting, exact combat economy, exact targeting taxonomy, or proprietary layout.
