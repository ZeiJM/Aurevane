...
begin
  if p_event_key is null
    ...
    or coalesce(p_definition ->> 'templateKey','') !~ '^[a-z0-9][a-z0-9._:-]{1,159}$'
    or coalesce(jsonb_typeof(p_definition -> 'contentVersion'),'') <> 'number'
    or coalesce(p_definition ->> 'contentVersion','') !~ '^[1-9][0-9]*$'
  ...
  loop
    if jsonb_typeof(v_phase) <> 'object'
      or coalesce(v_phase ->> 'id','') !~ '^[a-z0-9][a-z0-9._:-]{1,159}$'
    ...
    loop
      if jsonb_typeof(v_objective) <> 'object'
        or coalesce(v_objective ->> 'id','') !~ '^[a-z0-9][a-z0-9._:-]{1,159}$'
        or coalesce(v_objective ->> 'referenceKey','') !~ '^[a-z0-9][a-z0-9._:-]{1,159}$'
        ...
        or coalesce(v_objective ->> 'target','') !~ '^[1-9][0-9]*$'
    ...
    loop
      if jsonb_typeof(v_effect) <> 'object'
        ...
        or coalesce(v_effect ->> 'referenceKey','') !~ '^[a-z0-9][a-z0-9._:-]{1,159}$'
    ...
    if v_transition_type = 'manual' then
      ...
      if coalesce(jsonb_typeof(v_transition -> 'afterSeconds'),'') <> 'number'
        or coalesce(v_transition ->> 'afterSeconds','') !~ '^[1-9][0-9]*$'
        or (v_transition ->> 'afterSeconds')::numeric > 9007199254740991::numeric
      then
        raise exception using errcode = '22023', message = 'EVENT_DEFINITION_TRANSITION_INVALID';
      end if;
    elsif v_transition_type = 'scheduled' then
      if coalesce(v_transition ->> 'at','') = '' then
        raise exception using errcode = '22023', message = 'EVENT_DEFINITION_TRANSITION_INVALID';
      end if;
      begin
      ...
      if coalesce(v_transition ->> 'objectiveId','') !~ '^[a-z0-9][a-z0-9._:-]{1,159}$'
        or not ((v_transition ->> 'objectiveId') = any(v_phase_objective_ids))
      then
        raise exception using errcode = '22023', message = 'EVENT_DEFINITION_TRANSITION_INVALID';
      end if;
    else
  ...
  for v_ref in
    select value
    from jsonb_array_elements(p_definition -> 'rewardPackageRefs')
    union all
    select value
    from jsonb_array_elements(p_definition -> 'aftermathRefs')
  loop
    if jsonb_typeof(v_ref) <> 'string'
      or coalesce(v_ref #>> '{}','') !~ '^[a-z0-9][a-z0-9._:-]{1,159}$'
    then
