...
begin
  if coalesce(new.definition ->> 'schemaVersion', '') <> '1'
    or coalesce(new.definition ->> 'rewardPackageRef', '') <> new.reward_package_ref
    or coalesce(new.definition ->> 'packageVersion', '') !~ '^[1-9][0-9]*
  ...
  if coalesce(jsonb_typeof(v_reward), '') <> 'object'
    or coalesce(v_reward ->> 'type', '') <> 'character-xp'
    or coalesce(jsonb_typeof(v_reward -> 'amount'), '') <> 'number'
    or coalesce(v_reward ->> 'amount', '') !~ '^[1-9][0-9]{0,18}
...
commit;
    ...
    or coalesce(new.definition ->> 'budgetRef', '') <> new.budget_ref
    or coalesce(jsonb_typeof(new.definition -> 'rewards'), '') <> 'array'
