...
- [x] Read existing rules and add meaningful regression cases before implementation.
- [x] Implement engine and integration types with bounded immutable state.
- [x] Test multi-command preview/commit, same-team terrain, expiry, invalid pushes, hidden direct vs area targets, consumptions, JSON reload and terminal boundaries; run game-core tests and repo typecheck.
- [x] Commit and write a concise report explaining exact new public interfaces and test evidence.
...
- [x] Inspect Task 1 report and repository publication/snapshot resolution.
- [x] Publish changed definitions as new versions, migration, and content/compatibility tests.
- [x] Run focused game-core/server and database validation and report results.
- [x] Commit and document exact IDs/versions and migration/deploy ordering.
...
- [x] Read scoped battle AGENTS.md and current shared component contracts.
- [x] Implement intent routing and complete readable forecast/terrain/status presentation.
- [x] Verify Atlas inside Discipline Management and fix concrete functional/accessibility gaps.
- [x] Run relevant component/server tests, typecheck, and CI-ready browser scenarios; commit report.
