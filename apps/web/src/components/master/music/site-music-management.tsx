...
export function SiteMusicManagement({ initialConfig }: SiteMusicManagementProps) {
  ...
  async function upload(file: File, target: UploadTarget) {
    ...
    try {
      ...
      form.set('file', file)
      const response = await fetch('/api/master/site-music', { method: 'POST', body: form })
      if (!response.ok) throw new Error(await responseMessage(response))
      const payload = (await response.json()) as { track: SiteMusicTrack }
      if (target.kind === 'default') {
        updateDefaultTrack(payload.track)
      } else {
        updateOverride(target.id, (entry) => ({ ...entry, track: payload.track }))
      }
      setMessage(`${payload.track.label} uploaded. Save changes to publish it.`)
    } catch (error) {
  ...
  async function save() {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent(SITE_MUSIC_UPDATED_EVENT, { detail: payload.config }),
      )
    ...
    <section className={styles.root} aria-labelledby="site-music-heading">
      ...
      <div className={styles.rulesHeading}>
        <div>
          ...
          <p>
            Examples: <code>/auth</code>, <code>/game/character</code>, or{' '}
            <code>/game/battle</code>. Disable a rule to make that area silent.
          </p>
        </div>
        ...
        {config.routeOverrides.map((entry, index) => (
          <article className={styles.card} key={entry.id}>
            ...
            <TrackEditor
              track={entry.track}
              onChange={(track) =>
                updateOverride(entry.id, (current) => ({ ...current, track }))
              }
