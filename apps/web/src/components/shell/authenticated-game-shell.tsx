...
export function AuthenticatedGameRecovery() {
  return (
    <AuthenticatedShellFrame sessionLabel="Service recovery" footerLabel="Private game state unavailable">
      <Surface className={styles.primaryCard} tone="elevated">
        ...
        <p className={styles.lead}>
          AUREVANE verified your sign-in, but it could not safely load the private account and character state required to continue. No character or progression state was changed.
        </p>
        <div className={styles.characterState} data-testid="persistence-recovery">
          ...
          <p>Retry the private-state load, or use Account to sign out. AUREVANE will not create partial character state to bypass the problem.</p>
          <form action="/game" method="get"><button type="submit">Retry private-state load</button></form>
        </div>
      </Surface>
    ...
    <div className={styles.shell} data-testid="authenticated-shell">
      ...
      <header className={styles.masthead}>
        <div className={styles.brandGroup}>
          {backHref ? <Link className={styles.backButton} href={backHref} aria-label={backLabel ?? 'Back'}>←</Link> : null}
          <Link className="brand" href="/game/character" aria-label="AUREVANE character profile">
            ...
            <span className="brand__wordmark"><strong>AUREVANE</strong><small>Persistent tactical fantasy</small></span>
          </Link>
        </div>
        <span className={styles.sessionState}><StatusMark /> {sessionLabel}</span>
        <AccountMenu />
      </header>

      <main className={styles.main} id="game-main">{children}</main>
