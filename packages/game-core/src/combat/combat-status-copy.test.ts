...
import {
  createCombatEncounterState, evaluateCombatAction, executeCombatAction,
  endCombatTurn, validateCombatEncounterState,
  type CombatActionDefinition, type CombatContentCatalog, type CombatEncounterState,
  type CombatResolutionContext, type CombatStatusDefinition, type CombatStatusInstance,
} from './actions'
import { createPendingBattle, startBattle, selectFinalFacing, advanceBattleRng } from './battle-state'
...
import {
  createCombatActionProvenance, createCombatEffectInstanceProvenance,
  createCombatTriggerGuard, validateCombatEffectInstanceProvenance,
} from './combat-kernel-types'
import { normalizeCombatEffectState } from './combat-effect-state'

const POSITIVE: CombatStatusDefinition = {
  id: 'test.guard', version: 1, maximumStacks: 3, durationOwnerTurnStarts: 4,
  damageTakenMultiplierBasisPoints: 8_000, polarity: 'positive',
  amplifyCopyable: true, reactionClass: 'ordinary',
}
const NEGATIVE: CombatStatusDefinition = {
  id: 'test.blind', version: 1, maximumStacks: 1, durationOwnerTurnStarts: 4,
  damageTakenMultiplierBasisPoints: 10_000, polarity: 'negative',
  curseCopyable: true, reactionClass: 'ordinary', blindAccuracyPenaltyBasisPoints: 1_500,
}
const MARK: CombatStatusDefinition = {
  ...NEGATIVE, id: 'test.mark', blindAccuracyPenaltyBasisPoints: undefined,
  markAccuracyBonusBasisPoints: 1_500,
}
const REFLECT: CombatStatusDefinition = {
  ...POSITIVE, id: 'test.reflect', maximumStacks: 1,
  damageTakenMultiplierBasisPoints: 10_000, reactionClass: 'reactive', reflectBasisPoints: 2_500,
}
...
function row(definition: CombatStatusDefinition, patch: Partial<CombatStatusInstance> = {}): CombatStatusInstance {
  return {
    statusId: definition.id, statusVersion: definition.version,
    stacks: 1, remainingOwnerTurnStarts: 2, sourceCombatantId: 'other',
...
function world(
  ...
  const battle = startBattle(createPendingBattle({
    battleId: 'battle:status-copy', rulesVersion: 2, contentVersion: 2, rngSeed: 53,
    combatants: ids.map((id, index) => ({
      id, teamId: id === 'target' ? 'enemies' : 'players', initiative: 30 - index * 10,
      baseMovementBudget: 3, hp: 100, maxHp: 100, mp: 20, maxMp: 30,
    })),
  })).state
  return createStatDrivenCombatEncounterState(createCombatEncounterState(createTacticalBattleState({
  ...
  }), initial), ids.map((combatantId) => ({
    combatantId, provenance: { kind: 'scenario', sourceId: `scenario:${combatantId}`, sourceRulesVersion: 2 },
    accuracy, evasion: 0, armor: 0, ward: 0, jump: 0, physicalPower: 20, mysticPower: 20,
  })))
}
function attack(): CombatActionDefinition {
  return {
    id: 'test.hit', version: 1, sourceType: 'test', tags: [],
    target: { kind: 'unit', teamPolicy: 'any', shape: { kind: 'single' },
      minimumRange: 0, maximumRange: 3, requiresLineOfSight: false,
      maximumElevationDifference: null, friendlyFire: 'all-units' },
    cost: { spendsAction: false, mp: 0 }, requirements: [],
    effects: [{ type: 'damage', recipient: 'primary-unit', amount: 20 }],
  }
}
function copying(mode: 'amplify' | 'curse', patch: Partial<CombatActionDefinition> = {}): CombatActionDefinition {
  // The test-first fixture intentionally describes the new operation before its union exists.
  return { ...attack(), id: `test.${mode}`, cost: { spendsAction: true, mp: 4 },
    ...patch, effects: [{ type: 'copy-statuses', recipient: 'primary-unit', mode }] } as unknown as CombatActionDefinition
}
function statuses(state: CombatEncounterState, id: string) {
  return state.statusState.find((entry) => entry.combatantId === id)?.statuses ?? []
}
function context(actionId = 'test.amplify', actorId = 'actor', chain = 'chain:copy'): CombatResolutionContext {
  return {
    provenance: createCombatActionProvenance({ rulesetVersion: 2, sourceKind: 'test', actionDefinitionId: actionId,
      actionVersion: 1, sourceCombatantId: actorId, controllerCombatantId: actorId, triggerChainId: chain }),
...
function origin(targetCombatantId: string, effectOrdinal = 0, chain = 'chain:original') {
  return createCombatEffectInstanceProvenance({ action: context('test.original', 'other', chain).provenance,
    targetCombatantId, effectOrdinal, createdRound: 1, createdTurn: 1 })
}
function advance(state: CombatEncounterState, content = CONTENT) {
  return endCombatTurn({ ...state, tactical: { ...state.tactical,
    battle: selectFinalFacing(state.tactical.battle, 'east').state } }, content).state
}
function cast(state: CombatEncounterState, mode: 'amplify' | 'curse', content = CONTENT, ctx?: CombatResolutionContext) {
...
describe('Amplify Curse active-status copying: public command contract', () => {
  it('copies positive target statuses onto the caster without removing originals', () => {
    const state = world([{ combatantId: 'target', statuses: [row(POSITIVE, { stacks: 2 }), row(NEGATIVE)] }])
    const before = JSON.stringify(state)
    const result = cast(state, 'amplify')
    expect(statuses(result.state, 'actor')).toEqual([row(POSITIVE, { stacks: 2, sourceCombatantId: 'actor' })])
  ...
  it('copies negative caster statuses onto the selected target and leaves the caster afflicted', () => {
    ...
    expect(statuses(result.state, 'target')).toEqual([row(NEGATIVE, { sourceCombatantId: 'actor' })])
  ...
  it.each([1, 2, 3, 4])('copies %s remaining owner ticks without refilling the duration', (remainingOwnerTurnStarts) => {
    const state = world([{ combatantId: 'target', statuses: [row(POSITIVE, { remainingOwnerTurnStarts })] }])
    expect(statuses(cast(state, 'amplify').state, 'actor')[0]?.remainingOwnerTurnStarts).toBe(remainingOwnerTurnStarts)
  })
  it('merges receiver stacks within the definition cap without shortening its longer duration', () => {
    const state = world([
      { combatantId: 'target', statuses: [row(POSITIVE, { stacks: 2, remainingOwnerTurnStarts: 1 })] },
      { combatantId: 'actor', statuses: [row(POSITIVE, { stacks: 2, remainingOwnerTurnStarts: 3 })] },
    ])
  ...
  it('copies a Mark as a relationship owned by the Curse caster, preserving other target sources', () => {
    ...
    const result = cast(state, 'curse')
    expect(statuses(result.state, 'target').map((entry) => entry.sourceCombatantId)).toEqual(['actor', 'other'])
    expect(statuses(result.state, 'actor')).toEqual(statuses(state, 'actor'))
  })
  it('collapses rebinding-equivalent source Marks to one copy with the longest remaining duration', () => {
    const state = world([{ combatantId: 'actor', statuses: [
      row(MARK, { sourceCombatantId: 'target', remainingOwnerTurnStarts: 1 }), row(MARK),
    ] }])
    expect(statuses(cast(state, 'curse').state, 'target')).toEqual([
  ...
  it('forecasts the actual receiving caster and copied current state without mutating RNG', () => {
    ...
    expect(preview.projectedEffects).toContainEqual(expect.objectContaining({ effectType: 'copy-statuses', combatantId: 'actor' }))
    expect(preview.projectedEvents).toContainEqual(expect.objectContaining({ event: 'status_applied', targetCombatantId: 'actor',
      sourceCombatantId: 'actor', statusId: POSITIVE.id, stacks: 2, remainingOwnerTurnStarts: 2 }))
    expect(JSON.stringify(state)).toBe(before)
  })
  it.each(['amplify', 'curse'] as const)('gates a missed hostile %s copy without avoiding its resource costs', (mode) => {
    ...
    expect(result.state.tactical.battle.combatants.find((unit) => unit.id === 'actor')?.mp).toBe(16)
    expect(result.state.tactical.battle.rng).toEqual(advanceBattleRng(state.tactical.battle.rng).state)
  ...
  it('copies a reactive status without triggering its reaction during the copy', () => {
    ...
    expect(copied.events.some((event) => event.event === 'damage_applied' || event.event === 'healing_applied')).toBe(false)
  ...
  it('expires a one-tick copy through the normal recipient lifecycle', () => {
    const state = world([{ combatantId: 'target', statuses: [row(POSITIVE, { remainingOwnerTurnStarts: 1 })] }])
  ...
  it('preserves unrelated typed effects, resource history, terrain and temporary Skills', () => {
    ...
    const effectState = normalizeCombatEffectState({ damageHistory: [{ combatantId: 'actor', round: 1, amount: 12 }],
      temporarySkills: [{ combatantId: 'target', skillId: 'test.temporary', contentVersion: 1, sourceCombatantId: 'other' }] })
...
describe('Status copying: fail-closed eligibility and staged scope', () => {
  it.each(['amplify', 'curse'] as const)('rejects an empty %s donor without spending anything', (mode) => {
    const state = world()
    const before = JSON.stringify(state)
    const preview = evaluateCombatAction(state, copying(mode), TARGET, CONTENT)
    expect(preview.legal).toBe(false)
    expect(preview.issues).toContainEqual(expect.objectContaining({ code: 'requirement-not-met', message: expect.stringMatching(/eligible/i) }))
    expect(() => cast(state, mode)).toThrow(/eligible/i)
    expect(JSON.stringify(state)).toBe(before)
  })
  const excluded = [
    { amplifyCopyable: false }, { amplifyCopyable: undefined }, { polarity: undefined },
    { polarity: 'negative' }, { polarity: 'neutral' }, { polarity: 'mixed' },
    { reactionClass: 'system' }, { reactionClass: 'self-cost' },
  ] as const
  ...
  it.each(['system', 'self-cost'] as const)('does not Curse a %s penalty even with copy permission set', (reactionClass) => {
    const system = { ...NEGATIVE, id: 'test.penalty', blindAccuracyPenaltyBasisPoints: undefined, reactionClass }
  ...
  it.each([{ statusVersion: 2 }, { stacks: 4 }, { remainingOwnerTurnStarts: 5 }])('rejects mismatched donor state %j', (patch) => {
  ...
  it('does not permit copying onto oneself to create stacks', () => {
    ...
    expect(evaluateCombatAction(state, copying('amplify'), { kind: 'unit', combatantId: 'actor' }, CONTENT).legal).toBe(false)
  })
  ...
  it.each(['invalid', null, 1])('rejects invalid copy mode %j', (mode) => {
    const action = { ...copying('amplify'), effects: [{ type: 'copy-statuses', recipient: 'primary-unit', mode }] } as unknown as CombatActionDefinition
  ...
  it('rejects mixed effect packages until ordered copying has a tested contract', () => {
    const action = { ...copying('amplify'), effects: [...copying('amplify').effects, ...attack().effects] }
    expect(() => validateCombatActionDefinition(action, CONTENT)).toThrow()
  })
  it('rejects area copying in this single-recipient slice', () => {
    const action = copying('amplify', { target: { ...attack().target, kind: 'ground-tile', shape: { kind: 'circle', radius: 1 } } })
    expect(() => validateCombatActionDefinition(action, CONTENT)).toThrow()
  })
})
...
describe('Status copying: K3 lineage and deterministic identity', () => {
  it('uses distinct copied-instance identities for multiple statuses in one block', () => {
    ...
    expect(copies.map((entry) => entry.provenance?.copiedFromInstanceId)).toEqual([
      originalGuard.provenance?.instanceId, originalReflect.provenance?.instanceId,
    ])
    for (const entry of copies) {
      expect(entry.provenance).toMatchObject({ targetCombatantId: 'actor', effectOrdinal: 0, action: ctx.provenance })
  ...
  it('records the prior recipient instance as inherited lineage when stacks merge', () => {
    ...
    const state = world([{ combatantId: 'actor', statuses: [receiver] }, { combatantId: 'target', statuses: [donor] }])
    expect(statuses(cast(state, 'amplify', CONTENT, context()).state, 'actor')[0]?.provenance).toMatchObject({
      copiedFromInstanceId: donor.provenance?.instanceId, inheritedFromInstanceId: receiver.provenance?.instanceId,
    })
  })
  it('does not reuse a donor provenance object when no K3 command context is supplied', () => {
    const state = world([{ combatantId: 'target', statuses: [row(POSITIVE, { provenance: origin('target') })] }])
    const result = cast(state, 'amplify')
  ...
  it('extends deterministic provenance with a validated child-copy ordinal', () => {
    const input = { action: context().provenance, targetCombatantId: 'actor', effectOrdinal: 0,
      createdRound: 1, createdTurn: 1, copyOrdinal: 1 }
    const copied = createCombatEffectInstanceProvenance(input)
    expect(copied).toMatchObject({ copyOrdinal: 1 })
    expect(copied.instanceId).toContain(':copy:1')
    expect(validateCombatEffectInstanceProvenance(copied)).toEqual([])
    expect(validateCombatEffectInstanceProvenance({ ...copied, copyOrdinal: 2 }).length).toBeGreaterThan(0)
  })
  it.each([-1, 0.5, Number.NaN, Number.POSITIVE_INFINITY])('rejects invalid copy ordinal %s', (copyOrdinal) => {
    const input = { action: context().provenance, targetCombatantId: 'actor', effectOrdinal: 0,
      createdRound: 1, createdTurn: 1, copyOrdinal }
