...
import {
  createCombatEncounterState, evaluateCombatAction, executeCombatAction, endCombatTurn,
  validateCombatEncounterState, type CombatActionDefinition, type CombatEncounterState,
  type CombatContentCatalog, type CombatResolutionContext,
} from './actions'
...
import { advanceCurrentPoisonMovement, currentPoisonInstance, removeCurrentPoisonState } from './combat-dots'
import { validateCombatActionDefinition } from './combat-authoring-validation'
import {
  createCombatActionProvenance, createCombatEffectInstanceProvenance, createCombatTriggerGuard,
} from './combat-kernel-types'

const TARGET = { kind: 'unit' as const, combatantId: 'target' }
const CONTENT: CombatContentCatalog = { statuses: [{
  id: 'test.negative', version: 1, maximumStacks: 1, durationOwnerTurnStarts: 3,
  damageTakenMultiplierBasisPoints: 10_000, polarity: 'negative', reactionClass: 'ordinary', curseCopyable: true,
}] }
function world(accuracy = 10_000): CombatEncounterState {
  ...
  const battle = startBattle(createPendingBattle({
    battleId: 'battle:poison-copy', rulesVersion: 2, contentVersion: 2, rngSeed: 53,
    combatants: ids.map((id, i) => ({ id, teamId: id === 'target' ? 'enemies' : 'players',
      initiative: 30 - i * 10, baseMovementBudget: 3, hp: 100, maxHp: 100, mp: 20, maxMp: 30 })),
  })).state
  return createStatDrivenCombatEncounterState(createCombatEncounterState(createTacticalBattleState({
    battle, width: 3, height: 1, terrains: [{ id: 'open', traversalCost: 1 }],
  ...
  })), ids.map((combatantId) => ({ combatantId,
...
function action(mode: 'curse' | 'amplify' = 'curse'): CombatActionDefinition {
  return { id: `test.${mode}`, version: 1, sourceType: 'test', tags: [],
    target: { kind: 'unit', teamPolicy: 'any', shape: { kind: 'single' }, minimumRange: 0,
      maximumRange: 3, requiresLineOfSight: false, maximumElevationDifference: null, friendlyFire: 'all-units' },
    cost: { spendsAction: true, mp: 4 }, requirements: [],
...
function poisonAction(flag: unknown = true): CombatActionDefinition {
  ...
  const effect = { type: 'poison' as const, recipient: 'primary-unit' as const,
    ...(flag === undefined ? {} : { curseCopyable: flag }) }
  return { ...action(), id: 'test.poison', cost: { spendsAction: false, mp: 0 }, effects: [effect] }
}
function context(actionId = 'test.curse', actorId = 'actor', chain = 'chain:curse'): CombatResolutionContext {
  return { provenance: createCombatActionProvenance({ rulesetVersion: 2, sourceKind: 'test',
    actionDefinitionId: actionId, actionVersion: 1, sourceCombatantId: actorId,
    controllerCombatantId: actorId, triggerChainId: chain }),
...
function poison(state: CombatEncounterState, id = 'actor', counter = 0, flag: unknown = true) {
  const result = executeCombatAction(state, poisonAction(flag), { kind: 'unit', combatantId: id }, CONTENT)
...
function cast(state: CombatEncounterState, ctx?: CombatResolutionContext, mode: 'curse' | 'amplify' = 'curse') {
...
function withOrigin(state: CombatEncounterState, id: string, chain: string) {
  const provenance = createCombatEffectInstanceProvenance({
    action: context('test.original', 'other', chain).provenance, targetCombatantId: id,
    effectOrdinal: 0, createdRound: 1, createdTurn: 1,
  })
  return { ...state, effectState: { ...state.effectState!, poison: state.effectState!.poison.map((entry) =>
    entry.targetCombatantId === id ? { ...entry, sourceCombatantId: 'other', sourceActionId: 'test.original', provenance } : entry) } }
}
...
describe('Curse Poison: explicit authored and persisted copy policy', () => {
  ...
  it('keeps the exact old Poison shape when copy permission is omitted', () => {
    ...
    const unflagged = { ...action(), id: 'test.poison', cost: { spendsAction: false, mp: 0 },
      effects: [{ type: 'poison' as const, recipient: 'primary-unit' as const }] }
    const result = executeCombatAction(world(), unflagged, { kind: 'unit', combatantId: 'actor' }, CONTENT)
    expect(instance(result.state)).toEqual({ targetCombatantId: 'actor', sourceCombatantId: 'actor',
      sourceActionId: 'test.poison', profileVersion: 1, movementRemainder: 0 })
  })
  it('reapplication replaces copy permission instead of retaining a previous opt-in', () => {
    expect(instance(poison(poison(world()), 'actor', 0, false))).toHaveProperty('curseCopyable', false)
  })
  const malformed = [null, 0, 1, 'true', {}, [], NaN]
  it.each(malformed)('rejects malformed authored copy permission %j at both action boundaries', (flag) => {
  ...
  it.each(malformed)('rejects malformed persisted Poison copy permission %j', (flag) => {
    ...
    const changed: CombatEncounterState = { ...state, effectState: { ...state.effectState!,
...
describe('Curse Poison: real command and saved-state behavior', () => {
  it.each([0, 1, 2, 3, 4])('copies current movement counter %s and keeps the original', (counter) => {
    const state = poison(world(), 'actor', counter)
    const before = JSON.stringify(state)
    const result = cast(state)
    expect(instance(result.state, 'target')).toMatchObject({ movementRemainder: counter,
      sourceCombatantId: 'actor', sourceActionId: 'test.curse', profileVersion: 1, curseCopyable: true })
  ...
  const pairs = [0, 1, 2, 3, 4].flatMap((donor) => [0, 1, 2, 3, 4].map((receiver) => ({ donor, receiver })))
  it.each(pairs)('reapplication keeps one recipient instance and its own counter: %j', ({ donor, receiver }) => {
    const state = poison(poison(world(), 'actor', donor), 'target', receiver, false)
    const result = cast(state, context())
    expect(result.state.effectState?.poison.filter((entry) => entry.targetCombatantId === 'target')).toHaveLength(1)
    expect(instance(result.state, 'target')).toMatchObject({ movementRemainder: receiver,
      sourceCombatantId: 'actor', sourceActionId: 'test.curse', curseCopyable: true })
  ...
  it('rejects self-copy without spending resources', () => {
    const state = poison(world())
    const before = JSON.stringify(state)
    expect(() => executeCombatAction(state, action(), { kind: 'unit', combatantId: 'actor' }, CONTENT)).toThrow()
    expect(JSON.stringify(state)).toBe(before)
  })
  it('does not roll or mutate state while previewing a legal Poison-only Curse', () => {
    ...
    expect(result.projectedEffects.some((effect) => effect.effectType === 'copy-statuses' && effect.combatantId === 'target')).toBe(true)
    expect(JSON.stringify(state)).toBe(before)
  })
  it('spends normal resources on a missed Curse but preserves both Poison histories', () => {
    const state = withOrigin(poison(poison(world(0), 'actor', 4), 'target', 2), 'target', 'chain:existing')
  ...
  it('keeps Automatic Hit automatic for Poison copying', () => {
    ...
    const result = executeCombatAction(state, { ...action(), accuracyMode: 'automatic' }, TARGET, CONTENT)
  ...
  it('rebinds source and preserves immediate donor plus prior receiver lineage', () => {
    ...
    const result = cast(state, context())
    expect(instance(result.state, 'target').provenance).toMatchObject({ effectOrdinal: 0, copyOrdinal: 0,
  ...
  it('assigns distinct ordinals when the same Curse copies an ordinary status and Poison', () => {
    ...
    const state = { ...base, statusState: base.statusState.map((row) => row.combatantId === 'actor' ? {
